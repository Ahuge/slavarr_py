...
class SonarrClient:
    ...
    async def get_series_by_tvdb_or_tmdb(
        self, tvdb_id: int | None, tmdb_id: int | None
    ) -> dict | None:
    ...
    async def get_history_for_series(
        self, series_id: int, page_size: int = 10
    ) -> list[dict]:
    ...
    async def set_quality_profile(
        self, series_id: int, quality_profile_id: int
    ) -> dict:
    ...
    async def get_releases_for_episode(self, episode_id: int) -> list[dict]:
        ...
        headers = {"X-Api-Key": self.api_key}
        r = await self._client.get(
            url, headers=headers, params={"episodeId": episode_id}
        )
        r.raise_for_status()
        data = r.json()
        return data if isinstance(data, list) else []
